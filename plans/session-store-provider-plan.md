...
- **`sessionStore.ts`** — public interface, exported as a types-only package subpath (like `featureGateProvider.ts`):
  ...
  export interface SessionStore<V> {
    ...
    rotate?(oldKey: string, newKey: string, value: V): Promise<void>; // atomic delete-old+set-new
  }
  ```
  **Post-implementation update:** `set`/`rotate` do NOT take a `ttlMs` argument. Every call site for a given namespace always passed the same constant (`authzCodeTimeoutMs`, `refreshTokenTimeoutMs`, or a hardcoded 24-day literal for client registrations), so threading it through every call was repetitive and let a call site silently pass the wrong constant. TTL is instead configured once, at construction time, via `createNamespacedStore(namespace, { ttlMs, maxSize? })` — the same place `maxSize` already lives. A custom provider does not receive `ttlMs` at all; it manages its own expiration policy, same as it manages its own bounding.
...
- **`inMemorySessionStore.ts`** (mirrors `serverFeatureGate.ts`) — `InMemorySessionStore<V>` **composes** (does not extend) one `ExpiringMap<string, V>` (`src/utils/expiringMap.ts`) internally:
  - Constructor takes `{ ttlMs: number; maxSize?: number }` and stores `ttlMs` as an instance field — every namespace uses a single constant TTL for its whole lifetime, so it's configured once here rather than per call (see the post-implementation update above).
  - `get`/`delete` — thin `Promise.resolve()`-wrapped pass-throughs.
  - `set(key, value)` — schedules the entry using the instance's `ttlMs` (the `ExpiringMap` ctor's `defaultExpirationTimeMs` becomes an unused-in-practice placeholder, required only because the ctor throws on `<= 0`).
  - `consume(key)` — `get` then `delete` with no `await` between them (single JS tick), returning the pre-delete value.
  - `rotate(oldKey, newKey, value)` — `delete(oldKey)` then `set(newKey, value)` using the instance's `ttlMs`, implemented eagerly (never left `undefined`) for exact parity with today's manual delete+set pairs in `token.ts`.
  - `maxSize` is passed straight through to `ExpiringMap` — only the `clientRegistration` namespace uses `maxSize: 10_000` today; the other four stay unbounded, matching current behavior. `maxSize` is NOT part of the `SessionStore` interface — it's `InMemorySessionStore`-specific; a custom backend manages its own bounding.
...
```ts
private readonly pendingAuthorizations: SessionStore<PendingAuthorization> = createNamespacedStore('pendingAuthorization', { ttlMs: getConfig().oauth.authzCodeTimeoutMs });
private readonly authorizationCodes: SessionStore<AuthorizationCode> = createNamespacedStore('authorizationCode', { ttlMs: getConfig().oauth.authzCodeTimeoutMs });
private readonly refreshTokens: SessionStore<RefreshTokenData> = createNamespacedStore('refreshToken', { ttlMs: getConfig().oauth.refreshTokenTimeoutMs });
private readonly refreshTokenIndex: SessionStore<string> = createNamespacedStore('refreshTokenIndex', { ttlMs: getConfig().oauth.refreshTokenTimeoutMs });
private readonly clientRegistrations: SessionStore<ClientRegistration> = createNamespacedStore('clientRegistration', { ttlMs: milliseconds.fromDays(24), maxSize: 10_000 });
```
**Post-implementation update:** all five TTL constants (including the 24-day client-registration literal) live in these `createNamespacedStore` calls, not at the handler call sites — see the `set`/`rotate` update in section 1.

Drop the `ExpiringMap` import and usage from this file. `setupRoutes()` (lines 83–104) keeps passing these five fields into `register`/`authorize`/`callback`/`token`/`revoke` unchanged — only the handler *signatures* change (`Map<K,V>`/`ExpiringMap<K,V>` → `SessionStore<V>`, keys are already `string` everywhere).
...
- **`authorize.ts`**: `pendingAuthorizations.set(authKey, {...})` → `await ...set(authKey, {...})` (no `ttlMs` argument — the store's configured TTL, sourced from `config.oauth.authzCodeTimeoutMs` at construction time in `provider.ts`, applies automatically), and **delete** the existing `setLongTimeout(() => pendingAuthorizations.delete(authKey), ...)` manual-expiry block entirely — the store now owns expiry. Remove the now-unused `setLongTimeout` import if nothing else in the file needs it.
- **`callback.ts`**: keep the existing `get` (early, for `tableauState` validation) as `await ...get(authKey)`, and the existing cleanup `.delete(authKey)` (success path only) as `await ...delete(authKey)`. **Do not** collapse this into `consume()` — several early-return error paths (invalid state, token-exchange failure, site mismatch) intentionally leave the pending authorization in place today so a client can retry the callback after a transient failure; `consume()` would silently remove that retry behavior. `authorizationCodes.set(...)` → `await ...set(authorizationCode, {...})` (TTL comes from the store's construction-time config, same `authzCodeTimeoutMs`), keep the existing `expiresAt` field on the stored value (belt-and-suspenders app-level check, independent of store TTL).
- **`token.ts`**: authorization-code grant — collapse the existing get+delete pair into one `await authorizationCodes.consume(code)` (this one genuinely is single-use, no retry semantics, RFC-required exactly-once use). Refresh-token issuance — `refreshTokens.set(...)`/`refreshTokenIndex.set(...)` drop their per-call `ttlMs` (the store is already configured with `refreshTokenTimeoutMs`), and their paired `setLongTimeout(...delete...)` manual-expiry blocks are deleted. Refresh-token rotation — replace the existing delete-old+set-new pairs for both `refreshTokens` and `refreshTokenIndex` with `await store.rotate(oldKey, newKey, value)` calls (no `ttlMs` argument). Invalid/expired early-return cleanup stays plain `await ...delete(...)`.
...
- **`register.ts`**: handler becomes `async`; `clientRegistrations.set(clientId, {...})` → `await ...set(clientId, {...})` (no `ttlMs` argument — the 24-day TTL constant that used to live in `provider.ts`'s `ExpiringMap` ctor now lands directly in `provider.ts`'s `createNamespacedStore('clientRegistration', { ttlMs: milliseconds.fromDays(24), maxSize: 10_000 })` call, not at this call site).
