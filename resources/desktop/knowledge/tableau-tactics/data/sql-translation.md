...
When refactoring a custom SQL datasource to native tables (see `expertise://tableau/tableau-tactics/data/datasources`), SQL-computed columns become Tableau calculated fields. This is the translation reference.
...
**Important:** NTILE range-mapping is an approximation, not exact. SQL NTILE assigns equal-count buckets; the range-mapping formula assigns equal-width buckets. For exact quintiles, use `RANK()` with `CEILING(RANK / (COUNT / 5))`, or — for distribution-based exact quantile cutoffs — use the LOD `PERCENTILE` pattern documented in `expertise://tableau/tableau-tactics/data/calc-fields`.
...
For the full set of round-trip normalizations Tableau applies to formula text on save, see `expertise://tableau/tableau-tactics/data/round-trip-normalization`.
...
For pure Tableau calc-field authoring (column structure, parameters, table calc internals), see `expertise://tableau/tableau-tactics/data/calc-fields`. For the datasource-refactor workflow itself (custom SQL → native tables with `object-graph` + `relationships`), see `expertise://tableau/tableau-tactics/data/datasources`.
...
6. **For round-trip-stability concerns** (formulas getting rewritten by Tableau on save), see `expertise://tableau/tableau-tactics/data/round-trip-normalization`.
