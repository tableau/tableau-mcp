...
Confirmed patterns for calculated fields, parameters, parameter controls, count of records, Percent of Total table calculations, and customized tooltips. All patterns validated via `tableau-get-workbook` observation.
...
**Critical rules:**
- `name` must use auto-generated format `[Calculation_<digits>]` — literal `Calculation`, one underscore, then a single contiguous run of digits, and nothing else. The digits are an opaque unique ID (any unique number works: timestamps, randoms, sequential). Custom names like `[Is Selected Genre]` are invalid and will fail outright. **Two-underscore variants like `[Calculation_20260407120000_001]` (timestamp + trailing suffix) are a separate failure mode**: the XML parser accepts them and the field shows up in `tableau-list-available-fields`, but Tableau's formula-validation UI layer treats the name as malformed and flags the field "invalid" in the Data pane even when the formula itself compiled fine. Always use a single underscore.
...
What you write with `tableau-apply-workbook` is not bit-exact what `tableau-get-workbook` returns. Tableau applies a handful of idempotent normalizations on save (formula caption→internal-name rewrites, attribute re-quoting, `<column>` alphabetical re-sort, lazy dependency-graph resolution). For the full reference and the multi-line `&#10;` escaping pattern, see `expertise://tableau/tableau-tactics/data/round-trip-normalization`.

**Authoring rule of thumb (most important):** author formulas with internal names (`SUM([M_Q1_SLS])`) rather than captions (`SUM([Sales])`). Caption-authored formulas can transiently flag "invalid" in the Data pane until Tableau's validation cycle completes. Internal-name formulas are immediately canonical.
...
When refactoring a custom SQL datasource to native tables, SQL-computed columns become Tableau calculated fields. The full translation reference (FIXED LOD vs. table calc choice, scalar/window function maps, NTILE alternatives, nested LOD patterns, XML formula escaping) lives in its own module: see `expertise://tableau/tableau-tactics/data/sql-translation`. The datasource-refactor workflow itself (`object-graph` + `relationships`) is in `expertise://tableau/tableau-tactics/data/datasources`.
...
If you only need to place a field on shelves without any calculation logic, see `expertise://tableau/tableau-tactics/viz/worksheets` instead.
