...
- **Waterfall:** steps dimension on Columns, measure on Rows → Quick Table Calc **Running Total** → mark type **Gantt Bar** → create `-[Measure]` and put it on **Size** (without the negative-size field the bars won't connect into the staircase); signed measure on Color for up/down.
