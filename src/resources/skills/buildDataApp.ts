...
- **Declare every external origin the app fetches at runtime.** If the app \`fetch()\`/XHRs any origin
  ...
  must be declared via the \`requestedOrigins\` param on \`upsert-data-app-files\` /
  ...
  \`requestedOrigins: "https://<service-host>"\` in the same tool call. The param **replaces** the whole
...
in \`requestedOrigins\` — treat each one as a prompt to declare that origin (re-passing the complete set,
