...
describe('KnowledgeMethods', () => {
  it('lists graphs with bearer auth', async () => {
    const listGraphs = vi.fn().mockResolvedValue({ graphs: [] });
    ...
    methods._apiClient = { listGraphs };

    expect((methods as any).listGraphs).toBeTypeOf('function');
    await expect((methods as any).listGraphs()).resolves.toEqual([]);

    expect(listGraphs).toHaveBeenCalledWith({
