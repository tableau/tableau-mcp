...
describe('listTemplatesTool', () => {
  ...
  it('should create a tool instance with correct properties', () => {
    ...
    expect(tool.description).toBe('Search and page templates from the current Desktop repository.');
  ...
  it('caps the fully serialized protected-only compact result at 16384 bytes', async () => {
  ...
  it('returns compact template summaries by default', async () => {
    const body = await getBody({});
  ...
  it('exposes custom provenance and avoids bundled metadata when a repository template shadows it', async () => {
    const originalRepositoryDir = process.env['TABLEAU_REPOSITORY_DIR'];
    const originalTemplatesDir = process.env['TEMPLATES_DIR'];
