...
describe('getWorksheetXmlTool with External Client API transport', () => {
  ...
  it('uses worksheet list + per-item document routes without fetching the whole workbook', async () => {
    ...
    const result = await callback(
      { session: '999', worksheetName: 'Sales by Region', mode: 'inline' },
...
const instanceFor = (server: MockExternalApiServer): ExternalApiInstance => ({
  ...
  token: 'valid-token',
  pid: 999,
  instanceId: 'inst-tool-worksheet',
  apiVersion: '1.0',
});
