...
vi.mock('../../../desktop/wrappers/listDashboards.js');
...
describe('listDashboardsTool', () => {
  ...
  it('reports an honest too-new endpoint error when the dashboard list route is absent', async () => {
    ...
    vi.spyOn(listDashboardsModule, 'listDashboards').mockResolvedValue(Err(routeMissing));
    ...
    expect(result.isError).toBe(true);
    invariant(result.content[0].type === 'text');
    expect(result.content[0].text).toContain('does not serve the dashboard list endpoint');
