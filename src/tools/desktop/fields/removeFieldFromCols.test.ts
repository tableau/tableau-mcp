...
import { getRemoveFieldFromColsTool } from './removeFieldFromCols.js';
...
describe('removeFieldFromColsTool', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('should create a tool instance with correct properties', () => {
    const tool = getRemoveFieldFromColsTool(new DesktopMcpServer());
    expect(tool.name).toBe('remove-field-from-cols');
  ...
  it('should return error when removeFieldFromCols throws', async () => {
    vi.mocked(existsSync).mockReturnValue(true);
  ...
  it('should pass arguments correctly to removeFieldFromCols', async () => {
    vi.mocked(existsSync).mockReturnValue(true);
    vi.mocked(readFileSync).mockReturnValue('<worksheet/>');
    vi.mocked(metadataModule.removeFieldFromCols).mockReturnValue(MODIFIED_XML);
    vi.mocked(writeFileSync).mockReturnValue(undefined);
