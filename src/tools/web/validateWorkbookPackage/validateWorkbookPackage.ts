...
\`requestedOrigins\`. The published app's Content-Security-Policy blocks undeclared origins, so this is a
best-effort nudge to declare them (via the \`requestedOrigins\` param on \`upsert-data-app-files\` /
    ...
    callback: async (args, extra): Promise<CallToolResult> => {
      return await validateWorkbookPackageTool.logAndExecute<ValidateWorkbookPackageResult>({
        ...
        callback: async () => {
          ...
          // requests and flag any not declared in the manifest's `requestedOrigins`. Best-effort and
