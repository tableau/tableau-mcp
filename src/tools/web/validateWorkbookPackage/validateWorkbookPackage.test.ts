...
describe('validateWorkbookPackageTool', () => {
  ...
  it('advises (ok:true, receipt issued) when code requests an origin absent from requestedOrigins', async () => {
    const appId = await createApp([
      {
        path: 'index.html',
  ...
  it('stays silent when the requested origin is declared in the manifest requestedOrigins', async () => {
    const appId = await createApp([
      {
        path: 'index.html',
        content: '<!doctype html><html><body><script src="src/app.js"></script></body></html>',
      ...
      {
        path: 'dataapp.json',
        content: '{"schemaVersion":1,"appName":"My App","requestedOrigins":"https://api.acme.org"}',
      },
    ]);
    const payload = await getData(await run({ appId, workbookName: 'My App' }));
    expect(payload.ok).toBe(true);
