...
// fetch/XHR/WebSocket at runtime are whatever it DECLARES in the manifest `requestedOrigins` (which the
...
//     receipt on its own.
//   - It intentionally does NOT auto-add detected origins: `requestedOrigins` gates a security boundary,
 ...
 * advisory warning per origin the code requests but `requestedOrigins` never declared. `requestedOrigins`
...
export function undeclaredOriginsCheck(
  ...
  const declaredSources = (requestedOrigins ?? '').trim().split(/\s+/).filter(Boolean);
  ...
  return [...undeclared.entries()]
    ...
    .map(
      ([origin, path]) =>
        ...
        'requestedOrigins — the published app runs under a Content-Security-Policy that will block ' +
        'requests to it, failing silently in the sandbox. If the app fetches this origin at ' +
        'runtime, declare it via the requestedOrigins param and re-validate; if it is not fetched ' +
