...
const SAMPLE_VALUES_ALIAS = 'SampleValues';
const MAX_SAMPLE_VALUES = 3;

export async function fetchSampleStringValues(
  vizqlDataServiceMethods: VizqlDataServiceMethods,
  datasource: Datasource,
  fieldsResult: FieldsResult,
): Promise<FieldsResult> {
  const stringFields = fieldsResult.fieldGroups
    .flatMap((group) => group.fields)
    .filter(
      (field): field is Field & { name: string } => field.dataType === 'STRING' && !!field.name,
    );
  ...
  const fetchOne = async (field: Field & { name: string }): Promise<string[] | null> => {
    const query: Query = {
      fields: [{ fieldCaption: field.name, fieldAlias: SAMPLE_VALUES_ALIAS }],
    };
    ...
    try {
      ...
      const rows = (result.value.data ?? []) as Array<Record<string, unknown>>;
      const distinct = new Set<string>();
      for (const row of rows) {
        const raw = row[SAMPLE_VALUES_ALIAS] ?? row[field.name];
  ...
  const samples = await Promise.all(stringFields.map((field) => fetchOne(field)));
  for (let i = 0; i < stringFields.length; i++) {
    const values = samples[i];
    if (values && values.length > 0) {
      stringFields[i].sampleValues = values;
    }
  }
