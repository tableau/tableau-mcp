...
/**
 * One distinct `{ message, available }` failure-reason pair within a window of
 * flow runs, with how many runs and which flows it affected. `available: false`
 * means Tableau could not determine the cause and `message` is a generic
 * placeholder.
 */
...
/**
 * Failures for a window of flow runs, grouped by reported message and availability
 * so a caller can tell one systemic problem from many unrelated ones without
...
/**
 * Group the window's failure reasons by reported message and availability.
 *
 ...
 * message plus its availability flag is the finest stable grain available to
 * group on.
 *
