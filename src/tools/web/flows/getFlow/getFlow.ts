...
export const getGetFlowTool = (server: WebMcpServer): WebTool<typeof paramsSchema> => {
  ...
  **Returned Fields**
  ...
  - \`mcp.failureSummary\` (only when the returned runs include a \`Failed\` run carrying a \`failureReason\`): that window's failures, grouped by reported message and availability. \`failedRunCount\` and \`failedFlowCount\` cover ALL \`Failed\` runs returned; \`reasons\` holds one entry per distinct \`{ message, available }\` pair, biggest first, as \`{ message, available, runCount, flowIds }\` where \`flowIds\` are the DISTINCT flows that pair affected. A group is not always a single root cause — different underlying errors can share one short localized message. Identical in shape to the field \`list-flow-runs\` returns. Because the counts cover all failed runs while \`reasons\` covers only reason-bearing ones, \`sum(reasons[].runCount)\` can be less than \`failedRunCount\`. This tool emits no run-history deep link of its own: for an \`available: false\` group, append \`/runHistory\` to this response's \`webpageUrl\` to reach the flow's run-history page and read the error there.
