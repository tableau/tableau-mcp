...
describe('describeFlowTool', () => {
  ...
  it('renders mapped MCP errors through getErrorText()', async () => {
    const getErrorTextSpy = vi
      .spyOn(FlowDocumentForbiddenError.prototype, 'getErrorText')
      .mockReturnValue('custom forbidden guidance');
    mocks.mockGetFlowDocument.mockRejectedValue(axiosError(403, '403126'));
    ...
    try {
      ...
      expect(result.isError).toBe(true);
      invariant(result.content[0].type === 'text');
      expect(result.content[0].text).toBe('custom forbidden guidance');
    } finally {
      getErrorTextSpy.mockRestore();
    }
  });
