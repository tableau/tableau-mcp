...
export interface BuildTwbxInput {
  ...
  /** Space-separated CSP origin sources injected into the package manifest's `requestedOrigins` key,
   *  which the server prepends to the served content CSP's `default-src` (see monolith PR #61836; the
   *  key was renamed requestedOrigins -> requestedOrigins in W-24046733 / PR #64534). This is what lets
   *  the embedded app `fetch`/XHR to an origin other than its own isolated content origin. Omit (or
   *  pass blank) for none — the key is then absent and the manifest stays byte-identical. */
...
function renderManifest(i: BuildTwbxInput): string {
  ...
  // them). `requestedOrigins` is emitted ONLY when set (trimmed, non-empty): the server reads it to
  ...
  // The manifest KEY must be `requestedOrigins`: the monolith package parser reads exactly that field
  // (PackageManifestJsonParser -> optionalString(root, "requestedOrigins")) after the requestedOrigins ->
  // requestedOrigins rename (monolith W-24046733 / PR #64534). Emitting `requestedOrigins` here would be
  // silently ignored and the origins would never reach the served-content CSP.
  ...
  const origins = i.requestedOrigins?.trim();
  if (origins) {
