...
// workbook (verified against a real Sankey-extension .twb). The host worksheet is then wrapped in a
// single automatically-sized dashboard (renderDashboard) so the published workbook opens FULL-SCREEN
// on the viz instead of the bare worksheet — the same manual setup an author would otherwise do by
// hand (drop the sheet on a dashboard, set size = Automatic, hide the title, name the dashboard). The
// dashboard also gives the published workbook a second, meaningfully-named VIEW that publish links the
// result card to (see dashboardNameFor + toPublishResult), rather than the sheet's own view.
//
...
/** The dashboard's DISPLAY name for a given workbook. The published workbook wraps its single host
 *  worksheet in a full-screen dashboard; naming that dashboard deterministically (instead of letting
 *  Tableau fall back to its "Dashboard 1" auto-name, which is where the stray `Dashboard1` URL slug
 *  came from) gives the dashboard VIEW a meaningful contentUrl. This is the single source of truth for
 *  that name: the twb generator emits it as the <dashboard name>, and publish selects the dashboard
 *  view by this exact string (toPublishResult's preferredViewName) so the result card links to the
 *  dashboard rather than the sheet. The host worksheet keeps the plain workbookName. */
export function dashboardNameFor(workbookName: string): string {
  return `${workbookName} Dashboard`;
}
...
// The host worksheet's window. NOT maximized — the dashboard window (renderDashboardWindow) is the
// maximized one, so the published workbook opens on the full-screen dashboard, not the bare sheet.
// Cards are standard shelf layout; boilerplate.
function renderWorksheetWindow(seed: string, sheetName: string): string {
  return `    <window class='worksheet' name='${esc(sheetName)}'>
      <cards>
        <edge name='left'>
          <strip size='160'>
            <card type='pages' />
...
// The single, automatically-sized dashboard that wraps the host worksheet so the published workbook
// opens full-screen on the viz. Geometry/format are copied verbatim from a Tableau-authored
// single-sheet dashboard (a real .twb round-tripped through Desktop):
//   - <size sizing-mode='automatic'/>  → the dashboard fills whatever viewport renders it (the
//     "Automatic" size an author sets by hand).
//   - the outer layout-basic zone (id 4) fills the canvas (0,0 → 100000×100000); the sheet zone
//     (id 3) is inset by Tableau's default 8px (outer) / 4px (inner) margins.
//   - the sheet zone carries show-title='false' AND no title zone is emitted → BOTH the sheet title
//     and the dashboard title are hidden, so only the viz shows.
// The sheet zone's `name` MUST equal the host worksheet's name (the zone embeds that worksheet); the
// dashboard's own name comes from dashboardNameFor. <repository-location> (server-added on download)
// and <devicelayouts> (auto-generated by Desktop) are intentionally omitted — neither is needed to
// publish, and emitting them would only add non-load-bearing noise.
function renderDashboard(seed: string, dashboardName: string, sheetName: string): string {
  const dName = esc(dashboardName);
  const sName = esc(sheetName);
  return `  <dashboards>
    <dashboard enable-sort-zone-taborder='true' name='${dName}'>
      <style />
      <size sizing-mode='automatic' />
      <zones>
        <zone h='100000' id='4' type-v2='layout-basic' w='100000' x='0' y='0'>
          <zone h='97966' id='3' name='${sName}' show-title='false' w='98936' x='532' y='1017'>
            <zone-style>
      ...
      <simple-id uuid='{${uuidFor(`${seed}:dashboard`)}}' />
    </dashboard>
  </dashboards>`;
}

// The dashboard's window. Maximized so the published workbook opens straight onto the full-screen
// dashboard (which is why the host worksheet window is NOT maximized). The single <viewpoint> names
// the embedded host worksheet, and <active id='3'> selects the sheet zone declared in renderDashboard.
function renderDashboardWindow(seed: string, dashboardName: string, sheetName: string): string {
  return `    <window class='dashboard' maximized='true' name='${esc(dashboardName)}'>
      <viewpoints>
        <viewpoint name='${esc(sheetName)}' />
      </viewpoints>
      <active id='3' />
      <simple-id uuid='{${uuidFor(`${seed}:dashwin`)}}' />
    </window>`;
}
...
function renderTwb(i: BuildTwbxInput): string {
  ...
  // The host worksheet's display name is the workbook name; it is the view the viz extension mounts
  // on and the view the dashboard embeds. The referenced-view viewId below MUST match it. The
  // dashboard that wraps it takes the derived dashboardNameFor(...) name.
  const addInXml = renderAddIn(id, url, instanceId);
  const dashboardName = dashboardNameFor(i.workbookName);
...
${renderHostWorksheet(datasources, i.packageId, i.workbookName, addInXml)}${
  ...
  // The full-screen dashboard wrapping the host worksheet, plus its maximized window. Emitted between
  // <worksheets> and <windows> (the order Tableau serializes). The worksheet window stays non-maximized
  // so the workbook opens on the dashboard.
  const dashboardsXml = renderDashboard(i.packageId, dashboardName, i.workbookName);
  const dashboardWindow = renderDashboardWindow(i.packageId, dashboardName, i.workbookName);
  ...
  // at the host WORKSHEET (where the extension actually mounts), not the dashboard that embeds it.
  return `<?xml version='1.0' encoding='utf-8'?>
