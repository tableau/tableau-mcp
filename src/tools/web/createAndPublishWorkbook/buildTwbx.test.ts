...
describe('buildTwbx', () => {
  ...
  // The `requestedOrigins` INPUT is buildTwbx's param name; the emitted package-manifest KEY is
  // `requestedOrigins` (the wire name the monolith parser reads, post W-24046733 / PR #64534).
  describe('manifest requestedOrigins', () => {
    ...
    it('omits requestedOrigins entirely when unset (manifest keeps the base 4-field shape)', () => {
      ...
      expect('requestedOrigins' in manifest).toBe(false);
    });

    it('emits a single-origin string verbatim when set', () => {
      const manifest = manifestOf({ ...base, requestedOrigins: 'https://api.example.com' });
    ...
    it('treats a blank/whitespace value as unset (key omitted, output byte-identical to no-origins)', () => {
      const withBlank = buildTwbx({ ...base, requestedOrigins: '   ' }).bytes;
