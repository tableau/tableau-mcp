 ...
 * `dataapp.json` (`requestedOrigins`), which is protected from ordinary upserts. The data-app write
 * tools (`upsert-data-app-files`, `patch-data-app-file`) accept an optional `requestedOrigins` param
 ...
 */
export const requestedOriginsParam = z
  .string()
  .trim()
  .max(2000)
  .optional()
...
/**
 * Update ONLY the `requestedOrigins` key of the tool-managed `dataapp.json`, preserving every other
 ...
 * a scaffold's (the key is emitted last, and only when non-empty). `requestedOrigins` is the caller's
 ...
 */
export async function updateManifestRequestedOrigins(
