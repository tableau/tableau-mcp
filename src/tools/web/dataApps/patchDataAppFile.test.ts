...
describe('patchDataAppFileTool', () => {
  ...
  describe('requestedOrigins', () => {
    it('sets the manifest requestedOrigins alongside a patch, preserving every other field', async () => {
    ...
    it('clears the requestedOrigins key when an empty string is passed', async () => {
...
// setting/clearing requestedOrigins preserves every other field rather than clobbering a `{}` seed.
function realisticManifestJson(requestedOrigins?: string): string {
  ...
  return await callback(
    {
      ...
      ...(args.requestedOrigins !== undefined ? { requestedOrigins: args.requestedOrigins } : {}),
    },
    getMockRequestHandlerExtra(),
  );
}
