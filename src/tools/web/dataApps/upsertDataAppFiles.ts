...
const paramsSchema = {
  ...
  requestedOrigins: requestedOriginsParam,
};
...
declare it with the \`requestedOrigins\` param below instead of editing the manifest — do it in the
...
(required) — one or more \`{ path, content }\` entries; \`content\` is UTF-8 text. \`requestedOrigins\`
      ...
      return upsertDataAppFilesTool.logAndExecute<UpsertDataAppFilesResult>({
        ...
        callback: async () => {
          ...
          try {
            ...
            if (args.requestedOrigins !== undefined) {
              const manifestWrite = await updateManifestRequestedOrigins(
                store,
                scope.value,
                args.appId,
                args.requestedOrigins,
              );
