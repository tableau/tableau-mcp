...
import { requestedOriginsParam, updateManifestRequestedOrigins } from './manifestOrigins.js';
...
function manifestJson(requestedOrigins?: string): string {
...
describe('requestedOriginsParam', () => {
  it('is optional — undefined when absent', () => {
    expect(requestedOriginsParam.parse(undefined)).toBeUndefined();
  });

  it('trims surrounding whitespace', () => {
    expect(requestedOriginsParam.parse('  https://api.example.com  ')).toBe(
      'https://api.example.com',
    );
  });

  it('trims a whitespace-only value down to an empty string (the clear signal)', () => {
    expect(requestedOriginsParam.parse('   ')).toBe('');
  });

  it('rejects a value that exceeds the length cap after trimming', () => {
    expect(requestedOriginsParam.safeParse('x'.repeat(2001)).success).toBe(false);
    expect(requestedOriginsParam.safeParse('x'.repeat(2000)).success).toBe(true);
  });
});

describe('updateManifestRequestedOrigins', () => {
  async function seed(
    ...
    const ws = await store.create(scope, {
      ...
      packageId: 'com.example.myapp',
      template: 'live-extension',
      files: [{ path: DATA_APP_MANIFEST_PATH, content: manifestJson(requestedOrigins) }],
    });
  ...
  it('sets the origin while preserving every other manifest field, byte-for-byte', async () => {
    ...
    await updateManifestRequestedOrigins(store, scope, appId, 'https://api.example.com');
  ...
  it('clears the origin key when given a blank string', async () => {
    ...
    await updateManifestRequestedOrigins(store, scope, appId, '   ');
