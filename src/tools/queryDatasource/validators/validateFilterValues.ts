...
export async function validateFilterValues(
  ...
  const filtersToValidate = query.filters.filter(filter => 
  ...
  for (const filter of filtersToValidate) {
    ...
    try {
      if (filter.filterType === 'SET') {
        const result = await validateSetFilter(filter, fieldCaption, vizqlDataServiceMethods, datasource);
        if (result.isErr()) {
          validationErrors.push(result.error);
        }
      } else if (filter.filterType === 'MATCH') {
        const result = await validateMatchFilter(filter, fieldCaption, vizqlDataServiceMethods, datasource);
        if (result.isErr()) {
          validationErrors.push(result.error);
        }
      }
...
async function validateSetFilter(
  ...
  const existingValues = new Set(data.map((row: any) => String(row.DistinctValues || row[fieldCaption] || '')));
 ...
 */
function getFuzzyMatches(invalidValues: string[], existingValues: string[], maxDistance: number = 3, maxSuggestions: number = 5): string[] {
