...
describe('knowledge/search', () => {
  beforeEach(() => _resetKnowledgeSearchCache());
  ...
  ])('ranks the expected entry #1 for %j', (query, expectedSlugSuffix) => {
    const hits = searchKnowledge(query, 5);
    expect(hits.length).toBeGreaterThan(0);
    expect(hits[0].slug.endsWith(expectedSlugSuffix)).toBe(true);
  });
  ...
  ])('about-to-err query %j surfaces the corrective entry #1', (query, expectedSlugSuffix) => {
    const hits = searchKnowledge(query, 5);
    expect(hits.length).toBeGreaterThan(0);
    expect(hits[0].slug.endsWith(expectedSlugSuffix)).toBe(true);
  });
  ...
  ])('knowledge-route entry ranks #1 for its target ask %j', (query, expectedSlugSuffix) => {
    const hits = searchKnowledge(query, 5);
    expect(hits.length).toBeGreaterThan(0);
    expect(hits[0].slug.endsWith(expectedSlugSuffix)).toBe(true);
  });
  ...
  it.each([
    ...
    'next-lane knowledge-route entry ranks #1 for its target ask %j',
    (query, expectedSlugSuffix) => {
      const hits = searchKnowledge(query, 5);
      expect(hits.length).toBeGreaterThan(0);
      expect(hits[0].slug.endsWith(expectedSlugSuffix)).toBe(true);
    },
  ...
  ])('sankey propose-path entry ranks #1 for its target ask %j', (query, expectedSlugSuffix) => {
    const hits = searchKnowledge(query, 5);
    expect(hits.length).toBeGreaterThan(0);
    expect(hits[0].slug.endsWith(expectedSlugSuffix)).toBe(true);
  });
  ...
  ])('calc-shadow entry ranks #1 for its shadow-style ask %j', (query, expectedSlugSuffix) => {
    const hits = searchKnowledge(query, 5);
    expect(hits.length).toBeGreaterThan(0);
    expect(hits[0].slug.endsWith(expectedSlugSuffix)).toBe(true);
  });
  ...
  it.each([
    ...
    'sibling field-collision entry still ranks #1 for its ask %j (no cross-capture)',
    (query, expectedSlugSuffix) => {
      const hits = searchKnowledge(query, 5);
      expect(hits.length).toBeGreaterThan(0);
      expect(hits[0].slug.endsWith(expectedSlugSuffix)).toBe(true);
  ...
  ])('LOD membership tier calc entry ranks #1 for its ask %j', (query, expectedSlugSuffix) => {
    const hits = searchKnowledge(query, 5);
    expect(hits.length).toBeGreaterThan(0);
    expect(hits[0].slug.endsWith(expectedSlugSuffix)).toBe(true);
  });
