      ...
      if (workbookColumn) {
        ...
        throw new Error(
          `Column ${parsedCorrected.column} does not exist in datasource "${datasource}". ` +
            'Call resolve-field for a valid columnRef, or use a candidate field returned by ' +
            'bind-template. resolve-field re-reads the live workbook, so a field you just added ' +
            'in Desktop cannot be hidden by a stale cache.',
        );
      } else {
