...
export function guardCommand({
  ...
  const dialogPolicy = dialogPolicyFor(command);
  if (dialogPolicy) {
    return {
      refused: true,
...
function validateCommandRegistry({
  ...
  const dialogPolicy = dialogPolicyFor(command);
  if (dialogPolicy || !registry.invocable || registry.blockingDialog) {
    const reasons = [
      dialogPolicy ? 'live-observed dialog-popper' : undefined,
  ...
  for (const [key, value] of Object.entries(providedArgs)) {
    const param = findParam(registry, key);
    if (!param) {
  ...
  const missingRequired = registry.requiredParams.filter(
    (param) => !isContextFilledParam(param) && !hasParam(providedArgs, param),
  );
...
function hasParam(args: Record<string, unknown>, param: ExternalApiRegistryParam): boolean {
  return (
