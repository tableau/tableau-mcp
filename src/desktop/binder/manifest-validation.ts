// src/binder/manifest-validation.ts
//
// LOCKSTEP-CORE CANDIDATE (A↔B convergence, W11-C1 map §3). This module is the PURE
// half of the manifest layer: shape/enum validation plus the fixture-bind and
// fast-path eligibility predicates, all as plain functions over already-parsed objects.
//
// It has NO filesystem, NO provider/intelligence, and NO zod imports — the only
// dependency is the equally-pure `calc-derivation.ts` (also a lockstep-core candidate).
// The loader I/O (`manifest.ts`: fs reads, package-data-dir resolution, module cache)
// stays repo-specific and CALLS into this file; `manifest.ts` re-exports this module's
// public surface so existing importers of `./manifest.js` are unaffected.
//
// Keep this file byte-portable: do not add repo-specific imports here.
