...
export interface CachedPack {
  ...
  /** ISO-8601 timestamp of when the pack was written to cache (for TTL). */
  fetched_at: string;
}
...
/**
 * The full verification funnel (order matters — cheapest/structural first, crypto
 * last, and NEVER a partial read):
...
export function verifyPack(
  ...
  const parsed = parseSignedPackManifest(cached.signedManifest);
  if (parsed.isErr()) {
