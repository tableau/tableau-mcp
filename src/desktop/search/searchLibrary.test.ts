...
describe('searchWorkbookSchema', () => {
  ...
  it('rejects a well-formed override that is not an XSD schema', async () => {
    ...
    const nonSchemaPath = path.join(tempDir, 'not-schema.xsd');
    ...
    try {
      const { searchWorkbookSchema: isolatedSearch } = await import('./searchLibrary.js');
      expect(() => isolatedSearch({ elementType: 'anything' })).toThrow(
        /Workbook XSD root must be xs:schema/,
      );
    } finally {
      fs.rmSync(tempDir, { recursive: true, force: true });
    }
  ...
  it('rejects an XSD override with no indexed declarations', async () => {
    ...
    const emptySchemaPath = path.join(tempDir, 'empty.xsd');
    ...
    try {
      const { searchWorkbookSchema: isolatedSearch } = await import('./searchLibrary.js');
      expect(() => isolatedSearch({ elementType: 'anything' })).toThrow(
        /Workbook XSD contains no named declarations/,
      );
    } finally {
      fs.rmSync(tempDir, { recursive: true, force: true });
    }
