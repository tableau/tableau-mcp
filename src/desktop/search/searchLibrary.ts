...
function safeDeclarationLabel(name: string): string {
  ...
  return `name (${bytes} bytes, sha256:${digest})`;
}
...
function loadWorkbookSchema(): WorkbookSchemaIndex {
  ...
  const raw = overridePath
    ...
    : readDataAsset(WORKBOOK_XSD_ASSET);
  if (raw === null) {
  ...
  try {
    ...
    throw new Error(
      `Workbook XSD is not valid XML${overridePath ? ` at ${overridePath}` : ''}: ${message}`,
    );
  }
  if (!doc.documentElement || parseErrors.length > 0) {
    throw new Error(
      `Workbook XSD is not valid XML${overridePath ? ` at ${overridePath}` : ''}: ${parseErrors[0] ?? 'missing document element'}`,
    );
  }
  const rootKind =
  ...
  if (doc.documentElement.namespaceURI !== XSD_NAMESPACE || rootKind !== 'schema') {
    throw new Error(
      `Workbook XSD root must be xs:schema${overridePath ? ` at ${overridePath}` : ''}`,
    );
  }
  ...
  if (declarations.length === 0) {
    throw new Error(
      `Workbook XSD contains no named declarations${overridePath ? ` at ${overridePath}` : ''}`,
    );
  }
