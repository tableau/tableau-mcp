...
function ensureCommandsSearchIndex(): any {
  ...
  const recommendation: string =
    ...
    'Chart route precedence: preview/no-change and open multi-chart requests skip bind-template and use list-templates, list-available-fields, build-worksheets-from-templates, then apply-worksheet only when a write is requested. A recognizable single-view visualization request uses bind-template first: an explicit chart name may bind immediately; a semantic ask may return one bounded proposal. An explicitly named existing blank worksheet is a chart-creation target: use bind-template with target_worksheet. Populated worksheet edits use existing-sheet tools only: add-field then apply-worksheet for encodings; use refine-worksheet for top-N, sort, mark type, or operation=round_stacked_bar on a compatible stacked bar. For an unnamed derived metric, call author-calc before the artifact flow; use author-parameter, author-set, or author-action for those semantics.';
