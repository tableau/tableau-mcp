...
describe('DesktopConfig', () => {
  ...
  it('should create a config with default localExecutorConfig', () => {
    const config = new Config();
    expect(config.localExecutorConfig).toEqual({
  ...
  it('should set custom agentApiBase when specified', () => {
    ...
    const config = new Config();
    expect(config.localExecutorConfig.agentApiBase).toBe('http://localhost:9999/api/v2');
  });
  ...
  it('should set custom authToken when specified', () => {
    ...
    const config = new Config();
    expect(config.localExecutorConfig.authToken).toBe('test-token-123');
  });
  ...
  it('should set custom pollIntervalMs when specified', () => {
    ...
    const config = new Config();
    expect(config.localExecutorConfig.pollIntervalMs).toBe(5000);
  });
  ...
  it('should set pollIntervalMs to default when specified as a non-number', () => {
    ...
    const config = new Config();
    expect(config.localExecutorConfig.pollIntervalMs).toBe(1000);
  });
  ...
  it('should use maxRequestTimeoutMs for commandTimeoutMs', () => {
    ...
    const config = new Config();
    expect(config.localExecutorConfig.commandTimeoutMs).toBe(180000);
  });
});
