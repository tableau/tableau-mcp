...
describe('API-version tool gate (interim minApiVersion floor)', () => {
  ...
  it('the version-gated tools declare the expected floors', () => {
    ...
    expect(floors.get('run-workbook-optimizer')).toBe('0.2.11');
  });
  ...
  it('advertises Workbook Optimizer only at API 0.2.11 or newer in full and dynamic-authoring', () => {
    ...
    for (const profile of ['full', 'dynamic-authoring']) {
      ...
      expect(filterToolsByApiVersion(selected, '0.2.10').map((tool) => tool.name)).not.toContain(
        'run-workbook-optimizer',
      );
      expect(filterToolsByApiVersion(selected, '0.2.11').map((tool) => tool.name)).toContain(
        'run-workbook-optimizer',
      );
    }
