...
describe('desktop tools/list serialized surface', () => {
  it('keeps the served dynamic authoring profile under the tool-search auto-deferral threshold budget', async () => {
    ...
    // The dynamic surface carries desktop-read (served on the dynamic profile so agents reach
    // every read through one tool) plus the compact insight candidate tuples; the full surface
    // additionally carries the full-profile-only get-storyboard-xml and apply-storyboard tools.
    // Keep only a few bytes of ratchet headroom on each cap.
    expect(dynamicAuthoringTotal).toBeLessThanOrEqual(30_388);
...
describe('selectToolsForProfile (TOOL_PROFILE, W60 spike lever 1 / preamble P1)', () => {
  ...
  it('TOOL_PROFILE=dynamic-authoring registers exactly the 34-tool data-first singable surface — native authoring + workbook reads + the grouped environment read + atomic sheet activation + the manual path read/edit legs, no workbook round-trip/validation XML tools', () => {
  ...
  it('dynamic-authoring surface sits well under the 46k tools/list cliff (the whole point of a lean profile)', async () => {
    ...
    // Raised for the desktop-read dispatcher added to the served profile.
    expect(total).toBeLessThanOrEqual(30_388);
  });
