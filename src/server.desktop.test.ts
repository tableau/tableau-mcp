...
describe('desktop tools/list serialized surface', () => {
  it('keeps the served dynamic authoring profile under the tool-search auto-deferral threshold budget', async () => {
    ...
    // Honest wire measurements are 30,381 bytes dynamic and 43,254 bytes full: the full
    // surface dropped when nine environment/inspection reads folded into desktop-read, which
    // is served on the dynamic profile so agents reach every read through one tool.
