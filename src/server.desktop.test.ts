...
// Re-pinned 2026-08-19: apply-worksheet's worksheetName describe now states the name is inferred
// from a cached fragment (#756), +1 byte; dynamic authoring 40_081 -> 40_082 (budget -> 40_100,
// keeping the 18-char slack). Full surface 56_650 -> 56_651, still under the 56_668 budget ceiling.
const DYNAMIC_AUTHORING_SURFACE_EXPECTED = 40_082;
const DYNAMIC_AUTHORING_SURFACE_BUDGET = 40_100;
...
describe('desktop tools/list per-tool byte accounting', () => {
  ...
  const GRANDFATHERED: ReadonlyMap<string, number> = new Map([
    ...
    ['apply-worksheet', 1608], // +1 2026-08-19: worksheetName describe now states it is inferred from a cached fragment (#756); ratcheted down 2026-08-12 trimming the worksheetName describe to id-or-name; earlier raise 2026-08-10: direct templatePlan folds an exact single-view build into the existing guarded apply tool; no new tool surface
