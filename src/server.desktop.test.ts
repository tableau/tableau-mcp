...
For a clear derived-metric ask with no named chart type (margin %, ratio/rate/per, growth/change %), FIRST pass its conventional calc in bind-template's calcs[] and bind its caption in ONE call (for example, gross margin % = (SUM(Revenue)-SUM(COGS))/SUM(Revenue); a proposal still resolves via Call 2). Only after a formula/field-resolution failure, search-knowledge, then make ONE corrective bind-template call.
...
describe('desktop tools/list serialized surface', () => {
  it('keeps the served dynamic authoring profile under the tool-search auto-deferral threshold budget', async () => {
    ...
    // Honest wire measurements are 28,788 bytes dynamic and 44,270 bytes full.
    // Keep only a few bytes of ratchet headroom while staying well below the 46k cliff.
    expect(dynamicAuthoringTotal).toBeLessThanOrEqual(28_800);
...
describe('selectToolsForProfile (TOOL_PROFILE, W60 spike lever 1 / preamble P1)', () => {
  ...
  it('dynamic-authoring surface sits well under the 46k tools/list cliff (the whole point of a lean profile)', async () => {
    ...
    expect(total).toBeLessThanOrEqual(28_800);
  });
