...
Tableau MCP is a bridge between an AI agent and your Tableau Platform (APIs). It works with both Tableau Server and Tableau Cloud, but its capabilities will determined by the availability of the underlying Tableau services that the MCP tools depend on and an individual user's ability to access them. To get the most of Tableau MCP, we recommend the following:
