...
When `AUTH` is `uat`, the MCP server cannot sign a Tableau embed token from the UAT itself —
the Tableau Embedding API accepts only an embed-scoped JWT, which the UAT credential does not provide. To render the embedded Tableau viz
in the MCP app UI under UAT auth, provide a dedicated, optional Connected App (Direct Trust)
