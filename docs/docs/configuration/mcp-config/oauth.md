...
Only relevant when [`AUTH`](env-vars#auth) is `oauth` with the embedded authorization server
enabled (the default; see [`OAUTH_EMBEDDED_AUTHZ_SERVER`](#oauth_embedded_authz_server) above).
