...
- **Non-admin callers** — Tableau returns runs only for flows the caller has permission to view.
...
Multi-value lists use the form `flowId:in:[uuid1,uuid2]` or `status:in:[Failed,Cancelled]`. List
items are not quoted; commas inside list items are not supported.
